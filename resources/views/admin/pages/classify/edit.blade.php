...
<div class="content-wrapper">
    ...
    <section class="content">
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-6">
                    <form action="{{route('classify.update',$classify->id)}}" method="POST" role="from" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id=""
                                placeholder="Nhập tên name" name="name" value="{{$classify->name}}">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <select name="category_id" id="input" class="form-control" required="required">
                                ...
                                <option value="{{$value->id}}" {{($value->id == $classify->category_id)?'selected':''}}>{{$value->name}}</option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <div class="radio">
                                <label>
                                    <input type="radio" name="status" id="input" value="1"
                                        {{($classify->status==1)? 'checked':''}}>Hiện</input>
                                </label>
                            </div>
                            <div class="radio">
                                <label>
                                    <input type="radio" name="status" id="input" value="0"
                                        {{($classify->status==0)? 'checked':''}}>Ẩn</input>
                                </label>
                            </div>
                        </div>
