...
<div class="content-wrapper">
    ...
    <section class="content">
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-6">
                    <form action="{{route('category.update',$category->id)}}" method="POST" role="from" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id=""
                                placeholder="Nhập tên name" name="name" value="{{$category->name}}">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="file" class="form-control @error('name') is-invalid @enderror" id=""
                                placeholder="Nhập tên name" name="file" value="{{$category->logo}}">
                                <img src="{{url('upload')}}/{{$category->logo}}" alt="upload/{{$category->logo}}"
                                            width="100px">
                            ...
                            <div class="radio">
                                <label>
                                    <input type="radio" name="status" id="input" value="1"
                                        {{($category->status==1)? 'checked':''}}>Hiện</input>
                                </label>
                            </div>
                            <div class="radio">
                                <label>
                                    <input type="radio" name="status" id="input" value="0"
                                        {{($category->status==0)? 'checked':''}}>Ẩn</input>
                                </label>
                            </div>
                        </div>
